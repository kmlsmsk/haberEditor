...
(function() {     
    ...
    // 3. CSS STİLLERİ (FLEX YERİNE KESİN ABSOLUTE SANDVİÇ MİMARİSİ)
    ...
    style.innerHTML = `         
        ...
        .ne-bar-btn.active { background: #eff6ff; box-shadow: 0 4px 10px rgba(59, 130, 246, 0.15); border: 1px solid #bfdbfe; color: #3b82f6; }         
        ...
        /* 3. KISIM: FOOTER (SABİT ALT) */         
        .ne-footer {              
            position: absolute; bottom: 0; left: 0; right: 0;
            height: 65px; /* Kesin yükseklik */
            padding: 0 20px;              
        ...
        /* 2. KISIM: BODY (ARAYA SIKIŞTIRILMIŞ - KESİN SCROLL ALANI) */         
        .ne-body {              
            position: absolute; top: 60px; bottom: 65px; left: 0; right: 0;
            padding: 20px;              
            overflow-y: auto !important; /* CSS ezilmelerine karşı koruma */            
            overflow-x: hidden;             
            background: #ffffff;              
            display: block; /* Flex hatalarını önlemek için block yapıldı */        
        }                  
        
        /* Body Scrollbar (Görünür olması için belirginleştirildi) */         
        .ne-body::-webkit-scrollbar { width: 10px; }         
        .ne-body::-webkit-scrollbar-track { background: #f8fafc; border-radius: 10px; }         
        .ne-body::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 10px; border: 2px solid #f8fafc; }         
    ...
    bar.innerHTML = safeHTML(`         
        ...
        <div title="Haber Asistanı" style="margin-bottom: 4px; color: #3b82f6;">${ICONS.BOT_LOGO}</div>         
    ...
    modal.innerHTML = safeHTML(`         
        <div class="ne-head" id="ne-modal-head">             
            <div class="ne-title" style="color:#3b82f6;">${ICONS.BOT_LOGO} <span style="color:#0f172a;">Haber Asistanı</span></div>             
    ...
    // GÜVENLİ BODY EKLEMESİ
    const targetBody = document.body || document.documentElement;
    if (targetBody) {
        targetBody.appendChild(bar);
        targetBody.appendChild(modal);
    } else {
        console.warn("News Editor AI: Uygun bir HTML yapısı bulunamadı. Eklenti bu sayfada çalışamaz.");
        return; 
    }
    ...
    const els = {         
        ...
        bodyContainer: document.getElementById('ne-body-container') 
    };     
    let currentBase64 = '';      
    ...
    els.solve.onclick = () => {         
        ...
        chrome.storage.local.get(['geminiNewsKey'], async function(result) {             
            ...
            try {                 
                ...
                const promptText = ` Sen Ege Üniversitesi'nin haber ajansı olan Ege Ajans (euegeajans.com) için çalışan, son derece deneyimli, uzman bir haber muhabiri, yazar ve editörsün. Rolün profesyonel bir haber yazmaktır. Görevin, sana verilen ham metin, fotoğraf (varsa) ve referansları kullanarak profesyonel, akıcı ve doğrudan yayınlanmaya hazır, kusursuz bir haber metni oluşturmaktır.  Lütfen aşağıdaki kurallara ve formata harfiyen uy:  1. Referans Site: Haber formatı, dil ve üslup için 'https://www.euegeajans.com' adresindeki haberleri referans al. 2. Dil ve Üslup: Haberin konusuna göre (geçmiş olay, gelecek duyurusu vb.) doğru zaman kipini kullan. Metin akıcı, anlaşılır, kurumsal, pozitif ve çok detaylı olmalı. 3. Kişi Konuşmaları (Alıntılar): Kişilerin konuşmalarını her zaman doğrudan tırnak içinde ver ve cümlenin sonunu mutlaka "... dedi", "... ifadelerini kullandı" veya "... diye konuştu" şeklinde bağla. 4. Haber Yapısı ve Format:    - Haber girişine HER ZAMAN "İzmir (Ege Ajans)-" ekleyerek başla.    - Ana Başlık: Haberin en çarpıcı ve özetleyici cümlesi. (Markdown formatında '# Başlık' şeklinde ver)    - Üst Başlık (Spot): Ana başlığı destekleyen, habere dair merak uyandıran bir veya iki cümlelik özet. (Markdown formatında '**Kalın Yazı**' şeklinde ver)    - Ana Metin: Haberin tüm detaylarının anlatıldığı, paragraflara bölünmüş bölüm.    - Rektörün Görüşü: Ana metnin giriş paragrafından hemen sonra, Ege Üniversitesi Rektörü ${CFG.RECTOR}'nın konuyla ilgili bir demecine mutlaka yer ver. Bu demeci, verdiğim bilgilere ve haberin ruhuna uygun şekilde profesyonelce sen kurgulayacaksın.    - Ara Başlık(lar): Uzun metinlerde, konuyu bölümlere ayırmak için anlamlı ara başlıklar kullan. (Markdown formatında '## Ara Başlık' şeklinde ver) 5. Çıktı Formatı: Çıktıyı doğrudan ve sadece Markdown formatında üret. Ekstradan sohbet cümlesi (örneğin "İşte haberiniz:") kullanma. Tüm metni iki yana yaslanmış okumaya uygun bir paragraf düzeninde ver.  Aşağıdaki örnek şablonu incele ve formatı buna benzet:  # EÜ’nün Teknopark Ortaklı Projesine TÜBİTAK’tan Destek **EÜ Su Ürünleri Fakültesinin “TÜBİTAK 2209-B” kapsamında 2025 yılında kabul edilen ilk ve tek projesi**  **İzmir (Ege Ajans)-** Ege Üniversitesi Su Ürünleri Fakültesi ile Ege Teknopark iş birliğinde... (Giriş paragrafı)  Proje ekibini makamında ağırlayan Ege Üniversitesi Rektörü Prof. Dr. Musa ALCI, “Ege Üniversitesinde öğrencilerin araştırma-geliştirme ve proje kültürü kazanmaları amacıyla hayata geçirilen ekosistem meyvelerini vermeye devam ediyor. Tüm akademisyenlerimizi tebrik ediyorum” dedi.  ## "Fakültenin ilk projesi" (Haberin devamı...)  --- Referans URL İçeriği (Varsa): ${urlContext}  İşlenecek Ham Metin: ${rawText}`;                  
